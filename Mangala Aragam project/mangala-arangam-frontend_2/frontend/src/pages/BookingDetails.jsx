  ...
  return (
    <div className="max-w-2xl mx-auto px-5 sm:px-8 py-10">
      ...
      <Reveal delay={0.1} className="bg-white border border-stone/10 rounded-2xl p-6 mt-6">
        ...
        {booking.ownerPhone && booking.status !== 'REJECTED' && booking.status !== 'CANCELLED' && (
          <>
            <div className="kolam-divider my-5" />
            <div className="bg-parchment rounded-xl px-4 py-3.5 flex items-center justify-between">
              <div>
                <p className="text-[11px] uppercase tracking-wide text-stone/40 mb-0.5">Hall Owner Contact</p>
                <p className="text-sm font-semibold text-stone">{booking.ownerName}</p>
              </div>
              <a
                href={`tel:${booking.ownerPhone}`}
                className="flex items-center gap-2 px-4 py-2 rounded-full bg-kumkum text-ivory text-sm font-semibold"
              >
                <FaPhone size={12} /> {booking.ownerPhone}
              </a>
            </div>
          </>
        )}
